...
class Color {
	...
	public function setStyle (style:String) : Color
	{
		...
		if (r1.match(style) != null)
		{
			...
			r = Math.min( 100, Std.parseInt( color[ 1 ], 10 ) ) / 100;
			g = Math.min( 100, Std.parseInt( color[ 2 ], 10 ) ) / 100;
			b = Math.min( 100, Std.parseInt( color[ 3 ], 10 ) ) / 100;
			return this;
			
		}
		// #ff0000
		var r1:EReg = ~/^\#([0-9a-f]{6})$/i;
		if (r1.match(style) != null)
		{
			var rgbExpr:EReg = ~/^\#([0-9a-f]{6})$/i;
			var color = rgbExpr.split(style);
		...
		// #f00
		var r1:EReg = ~/^\#([0-9a-f])([0-9a-f])([0-9a-f])$/i;
		if (r1.match(style) != null)
		{
			var rgbExpr:EReg = ~/^\#([0-9a-f])([0-9a-f])([0-9a-f])$/i;
			var color = rgbExpr.split(style);
