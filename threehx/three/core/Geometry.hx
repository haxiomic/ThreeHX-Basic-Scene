...
class Geometry {
	...
	public function computeVertexNormals(areaWeighted:Bool = false) {
		...
		for (f in 0...this.faces.length) {
			...
			face.vertexNormals[ 0 ] = this.__tmpVertices[ face.a ].clone();
			face.vertexNormals[ 1 ] = this.__tmpVertices[ face.b ].clone();
			face.vertexNormals[ 2 ] = this.__tmpVertices[ face.c ].clone();
	...
	public function handleTriangle(context:Geometry, a:Int, b:Int, c:Int, ua:Int, ub:Int, uc:Int){
	...
	public function makeGroups(usesFaceMaterial:Bool){
		//!# todo cleanup this does not work needs refactoring 
		var face:Face3;
		var materialIndex:Int;
		var numMorphTargets = this.morphTargets.length;
		var numMorphNormals = this.morphNormals.length;
		var geometryGroupCounter = 0;
		var hash_map:Map<Int,Dynamic>, groupHash:Map<Int,String>;
		this.geometryGroups = new Map<String, Geometry>();
		...
		for ( f in 0...this.faces.length ) {
				...
				if ( hash_map.exists(materialIndex) == false ) {
					hash_map[ materialIndex ].hash = materialIndex;
					hash_map[ materialIndex ].counter = 0;
				}
				groupHash = hash_map[ materialIndex ].hash + '_' + hash_map[ materialIndex ].counter;
				if ( geometryGroups.exists(groupHash) == false ) {
					geometryGroups[ groupHash ].faces3 = [];
					geometryGroups[ groupHash ].materialIndex = materialIndex;
					geometryGroups[ groupHash ].vertices = 0;
					geometryGroups[ groupHash ].numMorphTargets = numMorphTargets;
					geometryGroups[ groupHash ].numMorphNormals = numMorphNormals;
			...
			for ( (key in myMap.keys() ) {
				...
				this.geometryGroups[ key ].id = geometryGroupCounter ++;

				this.geometryGroupsList.push( this.geometryGroups[ key ] );
	...
	public function dispose() {
		//!# todo add event when dispatch event created
	}	
