...
class Object3D {
	...
	// for GLRendererpu
	//!# todo  evaluate weather we really want to add these values here
	...
	public function clone(object:Object3D = null, recursive:Bool = true):Object3D {
		...
		if ( recursive === true ) {
			...
			for (key in children.keys()) {
				object.add(children.get(key).clone());
			}
		}
